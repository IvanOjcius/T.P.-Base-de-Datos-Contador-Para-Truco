...
<body>
    ...
    <div class="tablero">
        ...
        <div class="equipo">
            <input type="text" id="nombre-nosotros" class="nombre-equipo" value="Nosotros" onchange="guardarEstadoLocal()">
        ...
        <div class="equipo">
            <input type="text" id="nombre-ellos" class="nombre-equipo" value="Ellos" onchange="guardarEstadoLocal()">
    ...
    <a href="anotador.php" style="margin-top: 20px; display: inline-block;">
  ...
  <script>
    ...
    // Detectar si la URL indica continuar o iniciar de cero
    const urlParams = new URLSearchParams(window.location.search);
    const modo = urlParams.get('modo');

    if (modo === 'continuar' && localStorage.getItem('partida_activa')) {
        const datos = JSON.parse(localStorage.getItem('partida_activa'));
        puntosNosotros = datos.puntosNosotros || 0;
        puntosEllos = datos.puntosEllos || 0;
        limitePuntos = datos.limitePuntos || 30;
        
        document.addEventListener("DOMContentLoaded", () => {
            document.getElementById('nombre-nosotros').value = datos.nombreNosotros || "Nosotros";
            document.getElementById('nombre-ellos').value = datos.nombreEllos || "Ellos";
    ...
    function guardarEstadoLocal() {
        const estado = {
            nombreNosotros: document.getElementById('nombre-nosotros').value || "Nosotros",
            puntosNosotros: puntosNosotros,
            nombreEllos: document.getElementById('nombre-ellos').value || "Ellos",
            puntosEllos: puntosEllos,
            limitePuntos: limitePuntos
        };
        localStorage.setItem('partida_activa', JSON.stringify(estado));
    }
    ...
    function verificarGanador(nombreEquipo, puntos) {
        if (puntos >= limitePuntos) {
            setTimeout(() => { 
                ...
                localStorage.removeItem('partida_activa'); // Limpia la partida guardada al terminar
            }, 100);
        }
    }
